  ...
  for param_group, shape in param_groups_and_shapes:
    master_param = nn.Parameter(
        _flatten_dense_tensors(
            [param.detach().float() for (_, param) in param_group]).view(shape))
...
def model_grads_to_master_grads(param_groups_and_shapes, master_params):
  ...
  for master_param, (param_group, shape) in zip(master_params,
    ...
    master_param.grad = _flatten_dense_tensors(
        [param_grad_or_zeros(param) for (_, param) in param_group]).view(shape)
...
def master_params_to_model_params(param_groups_and_shapes, master_params):
  ...
  for master_param, (param_group, _) in zip(master_params,
    ...
    for (_, param), unflat_master_param in zip(
        param_group, unflatten_master_params(param_group,
                                             master_param.view(-1))):
...
class MixedPrecisionTrainer:
  ...
  def master_params_to_state_dict(self, master_params):
    return master_params_to_state_dict(self.model, self.param_groups_and_shapes,
