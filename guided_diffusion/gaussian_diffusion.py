...
class GaussianDiffusion:
  ...
  def q_mean_variance(self, x_start, t):
    ...
    log_variance = _extract_into_tensor(self.log_one_minus_alphas_cumprod, t,
                                        x_start.shape)
  ...
  def p_mean_variance(self,
    ...
    assert (model_mean.shape == model_log_variance.shape == pred_xstart.shape ==
            x.shape)
    return {
        "mean": model_mean,
  ...
  def p_sample_loop(
    ...
    if noise_step > 0:
      indices = tqdm(list(range(self.num_timesteps))[:noise_step][::-1])
      time = th.tensor([noise_step - 1] * ref_img.shape[0],
                       device=ref_img.device)
      sample = self.q_sample(start_img, time)

    else:
      indices = tqdm(list(range(self.num_timesteps))[::-1])
      sample = th.randn_like(ref_img)

    for idx in indices:
      t = th.tensor([idx] * shape[0], device=sample.device)
      if ((skip > 0 and ((idx + 1) % skip == 0) or
           idx >= (self.num_timesteps - last))) and idx >= first:
        ...
        difference = ref_img - out['pred_xstart']
        norm = th.linalg.norm(difference, dim=[1, 2, 3])
        norm = norm.sum()
        norm_grad = th.autograd.grad(outputs=norm, inputs=sample)[0].detach()

        sample = out['sample'].detach()
        sample -= norm_grad * scale
      else:
        with th.no_grad():
          out = self.p_sample(
              model,
  ...
  def ddim_reverse_sample(
    ...
    eps = (_extract_into_tensor(self.sqrt_recip_alphas_cumprod, t, x.shape) * x
           - out["pred_xstart"]) / _extract_into_tensor(
