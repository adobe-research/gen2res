...
class TrainLoop:
  ...
  def __init__(
    ...
    if not self.finetune and self.resume_step:
  ...
  def _load_ema_parameters(self, rate):
    ...
    if ema_checkpoint:
      ...
      state_dict = dist_util.load_state_dict(ema_checkpoint,
                                             map_location=dist_util.dev())
  ...
  def log_step(self):
    ...
    if self.step_time == 0:
      ...
      self.last_step_time = ct
    logger.logkv("time est.(10k)",
                 str(datetime.timedelta(seconds=10000 * self.step_time)))

    steps_to_go = (self.save_interval - self.step -
                   self.resume_step) % self.save_interval
    logger.logkv("time est.(next ckpt)",
