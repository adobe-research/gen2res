...
def tensor2img(tensor, rgb2bgr=True, out_type=np.uint8, min_max=(0, 1)):
    ...
    """
    if not (th.is_tensor(tensor) or (isinstance(tensor, list) and all(th.is_tensor(t) for t in tensor))):
...
def set_realesrgan():
    ...
    if th.cuda.is_available(): # set False in CPU/MPS mode
        ...
        if not True in [gpu in th.cuda.get_device_name(0) for gpu in no_half_gpu_list]:
            use_half = True
...
def main():
    ...
    for data, dic in dataloader:
        ...
        sample = sample_fn(
            ...
            ref_img=data if data.shape[-1] == args.image_size else F.interpolate(data, (args.image_size, args.image_size)),
            last=args.last,
            skip=args.skip,
            scale=args.scale,
        ...
        if not args.input_aligned:
            ...
            if bg_upsampler is not None:
                ...
                bg_img = bg_upsampler.enhance(cv2.imread(dic['path'][0], cv2.IMREAD_COLOR), outscale=args.upscale)[0]
            else:
                bg_img = None
            end_time = time.time()
            ...
            cropped_face_t = img2tensor(restored_img / 255., bgr2rgb=True, float32=True)
            normalize(cropped_face_t, (0.5, 0.5, 0.5), (0.5, 0.5, 0.5), inplace=True)
            sample = cropped_face_t.unsqueeze(0)

        # for i in range(sample.shape[0]):
        save_image((sample[0:1] + 1) / 2., os.path.join(args.outputdir, 'output.png'))
        # tot = tot + 1
